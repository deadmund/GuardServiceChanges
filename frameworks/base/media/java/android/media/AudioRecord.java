    ...
    public void setRecordingState(int newState){
    	...
    	switch(mRecordingState){
    		...
    		case RECORDSTATE_STOPPED:
    			Log.d(TAG, "Removing mic,  object name: " + rx.getDeviceName() + "  ID: " + rx.getDeviceID()); 
    	        GuardServiceHelper.remoteExcProtectedActiveChange(GuardServiceHelper.getIGSInstance(), 
...
// begin WITH_TAINT_TRACKING
        ...
        applyTaint(audioBuffer);
        return ret;
// end WITH_TAINT_TRACKING  
    }
    
    
    private void applyTaint(short[] s){
    	int taint = applyTaintHelper();
    	
    	// Only taint the data
    	Taint.addTaintShortArray(s, taint);
    }
    
    private void applyTaint(byte[] b){
    	int taint = applyTaintHelper();
    	
    	// Only taint the actual data
    	Taint.addTaintByteArray(b, taint);
    }
    
    private void applyTaint(ByteBuffer bb){
    	int taint = applyTaintHelper();
		
		// Only taint the actual data
    	if(bb.isDirect()){
    		Taint.addTaintDirectByteBuffer(bb, taint);
    	}
    }
    ...
    private int applyTaintHelper(){
    	...
    	if(res){
			...
			Log.d(TAG, "Channel: " + chans[0] + " => sender device ID: " + senderID);
