...
public class AudioTrack
{
    ...
    private void setPlayState(int newState){
    	...
    	switch(mPlayState){
    		...
    		case PLAYSTATE_PAUSED:
    			Log.d(TAG, "PAUSE or STOP, Removing speaker,  object name: " + tx.getDeviceName() + "  ID: " + tx.getDeviceID()); 
    	        GuardServiceHelper.remoteExcProtectedActiveChange(GuardServiceHelper.getIGSInstance(), 
    	        		tx, GuardServiceHelper.REMOVE_ACTIVE_TX);
    	        
    	        // Not yet implemented
    	        // GuardServiceHelper.remoteExcProtectedPauseTimer();
    ...
    protected void setState(int state) {
    	Log.d("GuardServ", "Audio Track State Changed via setState function.  New State: " + state);
        mState = state;
    }
    ...
    public int write(byte[] audioData, int offsetInBytes, int sizeInBytes) {
        ...
        if(mDataLoadMode == MODE_STREAM){
        	GuardServiceHelper.remoteExcProtectedStartTimer(tx);
        }
