...
public class VibratorService extends IVibratorService.Stub
    ...
    public void vibratePattern(int uid, String packageName, long[] pattern, int repeat,
        ...
        try {
            if (false) { // Really android dev's?  if (false)?
                Slog.i(TAG, "vibrating with pattern");
            }
    ...
    public void cancelVibrate(IBinder token) {
        ...
        // so wake-lock calls will succeed
        long identity = Binder.clearCallingIdentity();
        try {
            synchronized (mVibrations) {
    ...
    private void doVibratorOn(long millis, int uid) {
        synchronized (mInputDeviceVibrators) {
            ...
            if (vibratorCount != 0) {
            	...
            	// automatically remove this sender from the active senders list 
            	// after millis time has elapsed.  // Right now the sender is removed
            	// from the active senders list in the doVibratorOff method below
